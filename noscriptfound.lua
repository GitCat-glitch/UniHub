...
Callback = function()
    ...
    -- find Interactable -> Gambling Island Ticket (the part instance)
    local interactable = workspace:FindFirstChild("Interactable")
    if not interactable then
        warn("Interactable folder missing")
        return
    end

    local ticketPart = interactable:FindFirstChild("Gambling Island Ticket")
    if not ticketPart or not ticketPart:IsA("BasePart") then
        warn("Ticket part not found or not a BasePart")
        return
    end

    -- store previous CFrame
    local prevCFrame = hrp.CFrame

    -- teleport slightly above the part so physics doesn't clip the player into it
    hrp.CFrame = ticketPart.CFrame + Vector3.new(0, 3, 0)
    wait(0.12) -- allow network/physics to replicate
